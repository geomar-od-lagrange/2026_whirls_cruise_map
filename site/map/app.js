  ...
  const otherTabs = [
    {
      ...
      render: (div) => {
        ...
        // via the returned setTracksOn. `deps` threads the app-level collaborators the
        // rows reconcile against (updateClock/live clock, the forecast-visibility and
        // hide-outliers module flags via getters/setters, and the style/label helpers)
        // so core/controls.js stays a leaf and never imports back from app.js.
        const inst = buildInstrumentRows(div, map, markerGroups, tracksOverlay, vessels, {
          updateClock,
          getClockMs: () => atTimeClockMs,
          tracksOn,
          setForecastBatchVisible: (fn) => { forecastBatchVisible = fn; },
          getHideOutliers: () => hideOutliers,
          setHideOutliers: (on) => { hideOutliers = on; },
