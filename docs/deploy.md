...
`./vendor/leaflet-1.9.4/…`), and the one external resource (the vessel-position API)
is absolute HTTPS. (There is no basemap tile source — the map ships tile-free to
