...
~1.2 s, from ~46 s), pinned to the scalar reference by a test. Forward-only, so the
build's backward hindcast keeps the scalar integrator. Pure numpy: no new dependency.
...
**2000 seeds** per POST, plus `horizon_h`/`mark_step_h` ranges that cap the per-seed
dot schedule. Because the batch advects vectorized (~1–2 s at the cap — see *Batched,
not looped* above), the cap is **not** a latency bound — the full cap finishes far
inside the gateway's 60 s timeout — but a **DoS/memory** bound: it caps the CPU of one
unauthenticated POST and the transient trajectory array the batch materialises
(~18 MB at 2000 drops × 48 h, scaling with cap × horizon). The seed cap has a **single
source of truth** in the API and is advertised by `GET /api/forecast/limits`
(`{"max_seeds": …}`); the deploy-tool client fetches it and rejects an over-cap
deployment up front — "too many drops … increase spacing or shorten the path" — rather
than firing a doomed POST or hardcoding its own copy of the number. A request that slips
past that check (limits probe unavailable) is still rejected server-side with a `422`,
whose validation message the client renders verbatim instead of a bare error.

Because a whole deployment now advects in ~1–2 s, the request is a **single POST** — no
result cache, single-flight, or client retry. (An earlier revision added that machinery
to survive the 60 s gateway timeout when the scalar per-seed loop could take ~46 s at
the cap; vectorizing the advection removed the timeout pressure with a >40× margin, so
the machinery — and the single-pod/single-worker assumption it embedded — went with it.)
